...
- [ ] Add Flyway `V5__Create_Role_And_Permission_Tables.sql`
- [ ] Add Flyway `V6__dml_seed_admin_role.sql` (ADMIN role + 18 permissions)
- [ ] Add domain: `Role` + `RoleRepository` + use cases
- [ ] Add application: `RoleDTO`, mapper, application service
- [ ] Add infra: JPA entities + repository impl
- [ ] Add presentation: controller + request/response DTOs
- [ ] Secure endpoints (ADMIN-only)
- [ ] Add tests

### Frontend

- [x] Refactor `RoleService` from localStorage to HttpClient REST calls
- [x] Ensure permission list shows exactly 18 (from `GET /permissions`)
- [x] Keep existing UI behavior in `RoleManagementComponent`
- [x] Update unit tests
